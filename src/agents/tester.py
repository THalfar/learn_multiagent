...
class Tester(BaseAgent):
    ...
    def _find_saved_model(self, output_dir: str) -> str:
        ...
        # Search recursively for .zip files (SB3 model format)
        zip_files = list(output_path.rglob("*.zip"))
        if not zip_files:
            return ""

        # Prefer "best_model.zip" if it exists, otherwise take the largest .zip
        for zf in zip_files:
            if zf.name == "best_model.zip":
                return str(zf)

        # Fallback: largest zip file (most likely the trained model)
        largest = max(zip_files, key=lambda f: f.stat().st_size)
        return str(largest)
    ...
    def generate_video_script(env_name: str, model_path: str, output_dir: str) -> str:
        ...
        This bypasses LLM code generation entirely - the script loads a saved
        SB3 model, wraps the env with RecordVideo, and runs evaluation episodes.
        """
        return f'''import os
import gymnasium as gym
...
if model is None:
    print("ERROR: Could not load model with any SB3 algorithm")
    env.close()
    exit(1)
...
# First run episodes through RecordVideo wrapper
n_episodes = 5
total_reward = 0
...
# Check what videos were created
video_files = [f for f in os.listdir(OUTPUT_DIR) if f.endswith(".mp4")]
print(f"VIDEOS_CREATED: {{len(video_files)}} files")
for vf in video_files:
    size = os.path.getsize(os.path.join(OUTPUT_DIR, vf))
    print(f"  {{vf}}: {{size/1024:.1f}} KB")
'''
    ...
    def __call__(self, state: dict) -> dict:
        ...
        if current_phase == "demo":
            ...
            # Check if model file exists on host (video_dir is the Docker mount point)
            host_model_path = ""
            if best_model_path:
                # best_model_path is a host path from optimization phase
                if os.path.isfile(best_model_path):
                    host_model_path = best_model_path

            if host_model_path:
                print("\n" + "=" * 70)
                print("[bold magenta]🎬 DETERMINISTIC VIDEO RECORDING[/bold magenta]")
                print("=" * 70)
                print(f"[magenta]   Model: {host_model_path}[/magenta]")
                print(f"[magenta]   Env: {current_env_name}[/magenta]")
                print(f"[magenta]   Output: {video_dir}[/magenta]")
                print("[magenta]   Mode: Deterministic (no LLM code generation)[/magenta]")
                print("=" * 70)

                # Docker sees the model at /workspace/output/best_model.zip
                ...
                try:
                    ...
                    if video_check["valid_videos"] > 0:
                        ...
                        print(f"[bold red]❌ VIDEO FAILED: No valid videos created[/bold red]")
                        if stderr:
                            print(f"[red]   stderr: {stderr[:500]}[/red]")
                        # Fall through to normal LLM flow as fallback
                        print(f"[yellow]   Falling back to LLM-generated code...[/yellow]")

                except subprocess.TimeoutExpired:
                    print(f"[bold red]⏰ Demo script timed out after {execution_timeout}s[/bold red]")
                    print(f"[yellow]   Falling back to LLM-generated code...[/yellow]")
                except Exception as e:
                    print(f"[bold red]❌ Demo script error: {e}[/bold red]")
                    print(f"[yellow]   Falling back to LLM-generated code...[/yellow]")
            else:
                print(f"[yellow]⚠️  No saved model found - using LLM-generated code for demo[/yellow]")
        ...
        try:
            ...
            if current_phase == "optimization":
                model_path = self._find_saved_model(video_dir)
                if model_path:
                    result_dict["best_model_path"] = model_path
